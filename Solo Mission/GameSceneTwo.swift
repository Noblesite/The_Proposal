...
class GameSceneTwo: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        for i in 0...1{
            ...
            let background = SKSpriteNode(imageNamed: "backgroundLevel3")
    ...
    func addScore(){
        ...
        if gameScore >= pastGameScore+120 {
            currentGameState = gameState.moving
            prepToMoveToNextLevel()
            
            
            
        }else if gameScore == pastGameScore+10 || gameScore == pastGameScore+25 || gameScore == pastGameScore+50{
            startAndSetEnemySpawn()
        }
        
        if gameScore == pastGameScore+25 || gameScore == pastGameScore+40 || gameScore == pastGameScore+60{
            startAndSetEnemyTwoSpawn()
        }
        
        if gameScore == pastGameScore+35 || gameScore == pastGameScore+60{
            spawnLife()
        }
    ...
    func spawnEnemy(){
        ...
        let enemy = SKSpriteNode(imageNamed: "level3Enime")
        enemy.name = "Enemy"
        enemy.setScale(0.15)
    ...
    func moveToNextGameScene(){
        ...
        let sceneToMoveTo = BossSceneTwo(size: self.size)
        sceneToMoveTo.scaleMode = self.scaleMode
        let myTransition = SKTransition.fade(withDuration: 0.5)
        self.view!.presentScene(sceneToMoveTo, transition: myTransition)
